...
import static com.leagueofshadows.enc.storage.SQLHelper.ENCRYPTED_MESSAGES_IS_GROUP_MESSAGE;
...
import static com.leagueofshadows.enc.storage.SQLHelper.GROUPS_ACTIVE;
import static com.leagueofshadows.enc.storage.SQLHelper.GROUPS_ADMINS;
...
import static com.leagueofshadows.enc.storage.SQLHelper.MESSAGES_IS_GROUP_MESSAGE;
...
public class DatabaseManager {
    ...
    public void insertNewMessage(Message message, String id, String currentUserId)
    {
        ...
        if(cursor.getCount()==0) {
            ...
            contentValues.put(MESSAGES_IS_GROUP_MESSAGE,message.getIsGroupMessage());
    ...
    public void updateMessage(Message message,String id)
    {
        String tableName;
        if(message.getIsGroupMessage()==Message.MESSAGE_TYPE_GROUP_MESSAGE)
            tableName = getTableName(message.getTo());
        else
            tableName = getTableName(id);
        ...
        if(cursor.moveToFirst())
        {
            do {
                Message message = new Message(cursor.getInt(0),cursor.getString(cursor.getColumnIndex(MESSAGES_ID)),cursor.getString(cursor.getColumnIndex(MESSAGES_TO))
                        ...
                        ,cursor.getString(cursor.getColumnIndex(MESSAGES_RECEIVED)),cursor.getString(cursor.getColumnIndex(MESSAGES_SEEN)),cursor.getInt(cursor.getColumnIndex(MESSAGES_IS_GROUP_MESSAGE)));
    ...
    public void updateMessageSeenStatus(String timestamp, String message_id,String otherUserId,String groupId,String currentUserId)
    {
        SQLiteDatabase database = openDatabase();
        if(groupId==null) {
           putSeenStatus(otherUserId,timestamp,message_id,database);
        }
        else {
            Message message = getMessage(message_id,groupId);
            if (!message.getFrom().equals(currentUserId)){
               putSeenStatus(groupId,timestamp,message_id,database);
            }
            else {
                insertGroupParticipantSeen(otherUserId, message_id, timestamp);
                ...
                raw = "SELECT * FROM " + TABLE_GROUP_PARTICIPANTS + " WHERE " + GROUP_PARTICIPANTS_GROUP_ID + " = ?";
    ...
    private void putSeenStatus(String tableName,String timestamp,String message_id,SQLiteDatabase database){
    ...
    private void insertGroupParticipantSeen(String userId, String messageId, String timestamp) {

        SQLiteDatabase sqLiteDatabase = openDatabase();
        String raw = "SELECT * FROM "+TABLE_MESSAGES_SEEN_STATUS+ " WHERE "+SEEN_STATUS_MESSAGE_ID+" = ? AND "+SEEN_STATUS_USER_ID + " = ?";
        Cursor cursor = sqLiteDatabase.rawQuery(raw,new String[]{messageId,userId});
        if(cursor.getCount()==0) {
            ...
            sqLiteDatabase.insert(TABLE_MESSAGES_SEEN_STATUS, null, contentValues);
        }
        else
            cursor.close();
    }
    ...
    public void updateMessageReceivedStatus(String timestamp, String message_id,String otherUserId,String groupId)
    {
        ...
        if(groupId==null) {
            ...
            insertReceivedStatus(otherUserId,message_id,timestamp);
    ...
    {
        SQLiteDatabase sqLiteDatabase = openDatabase();
        String raw = "SELECT * FROM "+TABLE_MESSAGES_RECEIVED_STATUS+ " WHERE "+RECEIVED_STATUS_MESSAGE_ID+" = ? AND "+RECEIVED_STATUS_USER_ID + " = ?";
        Cursor cursor = sqLiteDatabase.rawQuery(raw,new String[]{messageId,userId});
        if(cursor.getCount()==0) {
            ContentValues contentValues = new ContentValues();
    ...
    public Message getMessage(String messageId,String otherUserId)
    {
        ...
        if (cursor.getCount()!=0) {
            ...
            Message message = new Message(cursor.getInt(cursor.getColumnIndex(ID)), cursor.getString(cursor.getColumnIndex(MESSAGES_ID)), cursor.getString(cursor.getColumnIndex(MESSAGES_TO))
                    , cursor.getString(cursor.getColumnIndex(MESSAGES_FROM)), cursor.getString(cursor.getColumnIndex(MESSAGES_CONTENT)), cursor.getString(cursor.getColumnIndex(MESSAGES_FILEPATH))
                    , cursor.getString(cursor.getColumnIndex(MESSAGES_TIMESTAMP)), cursor.getInt(cursor.getColumnIndex(MESSAGES_TYPE)), cursor.getString(cursor.getColumnIndex(MESSAGES_SENT))
                    , cursor.getString(cursor.getColumnIndex(MESSAGES_RECEIVED)), cursor.getString(cursor.getColumnIndex(MESSAGES_SEEN)),cursor.getInt(cursor.getColumnIndex(MESSAGES_IS_GROUP_MESSAGE)));
            cursor.close();
            ...
            if (cursor.moveToFirst()) {
                do {
                    ...
                    EncryptedMessage encryptedMessage = new EncryptedMessage(cursor.getString(cursor.getColumnIndex(ENCRYPTED_MESSAGES_ID)), cursor.getString(cursor.getColumnIndex(ENCRYPTED_MESSAGES_TO))
                            ...
                            Boolean.parseBoolean(cursor.getString(cursor.getColumnIndex(ENCRYPTED_MESSAGES_TIMESTAMP))),cursor.getInt(cursor.getColumnIndex(ENCRYPTED_MESSAGES_IS_GROUP_MESSAGE)));
    ...
    public void insertEncryptedMessages(ArrayList<EncryptedMessage> encryptedMessages)
    {
        ...
        for (EncryptedMessage e:encryptedMessages) {
            ...
            if(cursor.getCount()==0) {
                ...
                contentValues.put(ENCRYPTED_MESSAGES_IS_GROUP_MESSAGE,e.getIsGroupMessage());
    ...
    public void addNewGroup(Group group)
    {
        ...
        contentValues.put(GROUPS_ADMINS,group.getAdmins());
        contentValues.put(GROUPS_ACTIVE,group.getGroupActive());
        if(cursor.getCount()==0) {
    ...
    public Group getGroup(String groupId)
    {
        ...
        if(cursor.getCount()!=0)
        {
            ...
            cursor1.close();
            return new Group(groupId,name,users,admins,groupActive);
        }
        cursor.close();
        return null;
    }

    public ArrayList<String> getGroupParticipants(String groupId){
        ArrayList<String> userIds = new ArrayList<>();

        SQLiteDatabase sqLiteDatabase = openDatabase();
        String rawQuery = "SELECT * FROM "+TABLE_GROUP_PARTICIPANTS+" WHERE "+GROUP_PARTICIPANTS_GROUP_ID+" = ?";
        Cursor cursor1 = sqLiteDatabase.rawQuery(rawQuery, new String[]{groupId});
        if(cursor1.moveToFirst()) {
            do {
                String userId = cursor1.getString(cursor1.getColumnIndex(GROUP_PARTICIPANTS_USER_ID));
    ...
    public void markGroupAsDeleted(String groupId){
        ...
        sqLiteDatabase.update(TABLE_GROUPS,contentValues,GROUPS_ID+" = ?", new String[]{groupId});
    }

    public void updateGroupUsers(Group group){

        String raw = "SELECT * FROM "+TABLE_GROUPS+" WHERE "+GROUPS_ID +" = ?";
        Cursor cursor = openDatabase().rawQuery(raw, new String[]{group.getId()});
        if(cursor.getCount()==0){
            addNewGroup(group);
        }
        else{
            cursor.close();
            SQLiteDatabase sqLiteDatabase = openDatabase();
            sqLiteDatabase.delete(TABLE_GROUP_PARTICIPANTS,GROUP_PARTICIPANTS_GROUP_ID +" = ?", new String[]{group.getId()});
    ...
    public void updateGroupName(String name,String groupId){
        SQLiteDatabase sqLiteDatabase = openDatabase();
        ContentValues contentValues = new ContentValues();
        contentValues.put(GROUPS_NAME,name);
        sqLiteDatabase.update(TABLE_GROUPS,contentValues,GROUPS_ID+" = ?", new String[]{groupId});
    }
        ...
        try {
            for (String userId : userIds) {
                ...
                if (cursor2.moveToFirst()) {
                    receivedTimestamp = cursor2.getString(cursor2.getColumnIndex(RECEIVED_STATUS_TIMESTAMP));
                    cursor2.close();
                }
